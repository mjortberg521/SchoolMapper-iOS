...
class secondFloorTabViewController: UIViewController, CLLocationManagerDelegate {
    ...
    var school = School(schoolName: "Glenbrook South High School") //the name corresponding to our school
    ...
    func formatOverlay(school: School) {
        school.midCoordinate = self.imageCoordinateDictSecond["midCoordinate"]! //make these school variables so the overlay works--the overlay's bounding box is created in school.swift
        school.overlayTopLeftCoordinate = self.imageCoordinateDictSecond["overlayTopLeftCoord"]!
        school.overlayTopRightCoordinate = self.imageCoordinateDictSecond["overlayTopRightCoord"]!
        school.overlayBottomLeftCoordinate = self.imageCoordinateDictSecond["overlayBottomLeftCoord"]!
    }
    ...
    override func viewDidLoad() {
        ...
        if UserDefaults.standard.value(forKey: "User School Name") == nil { //this would happen if the user switched to the map screen without selecting a school first
           ...
           //the userDefault should never be nil because the user should select one from the intiial table view
        }
        ...
        DispatchQueue.global(qos: .utility).async {
            print("this is on the background thread")
            
            repeat {
                print("not ready to go")
            }   while self.secondFloorplanImage?.size.height == nil || self.secondFloorplanImage?.size.width == nil || self.imageStringCoordinateDictSecond.isEmpty == true || self.latitudes_first.isEmpty == true || self.latitudes_second.isEmpty == true || self.longitudes_first.isEmpty == true || self.longitudes_second.isEmpty == true
            ...
            DispatchQueue.main.async {
                ...
                self.imageCoordinateDictSecond = self.self.stringDictToCoordinate(stringDict: self.imageStringCoordinateDictSecond) //create a dict of CLLocationCoordinates after waiting to recieve a string dictionary from firebase
                
                self.formatOverlay(school: self.school)
                
                let overlay = SchoolMapOverlay(school: self.school)
                self.secondFloorMapView.add(overlay)
                ...
                self.secondFloorMapView.setRegion(region, animated: false)
            }
        }
        ...
        imageRootRef.child("Second floor").observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            print(self.imageStringCoordinateDictSecond)
        })
        ...
        secondFloorMapView.showsPointsOfInterest = false
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if UserDefaults.standard.value(forKey: "User School Name") != nil {
            ...
            if UserDefaults.standard.value(forKey: "User School Name") as! String != schoolName {
                
                self.secondFloorplanImage = nil //dump the previous image
                ...
                DispatchQueue.global(qos: .utility).async {
                    ...
                    repeat { //hold up the background thread while we keep downloading on the main thread, then proceed to the .main.async once we're done with the while statement below
...
//MKMapViewDelegate
extension secondFloorTabViewController: MKMapViewDelegate {
    
    func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
        if overlay is SchoolMapOverlay {
            return SchoolMapOverlayView(overlay: overlay, overlayImage: secondFloorplanImage!) //#imageLiteral
        }
