...
class SourceDestViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        func addNeighbors(sourceNode:MyNode, array:[MyNode]) { //used to find the weight of the path between two nodes - array is the list of neighbors
            ...
            for node in array { //iterate over the list of neighbors
                ...
                if latitudes_first.keys.contains(y) { //checks if the node being connected to is upstairs or downstairs so it knows which plist to look in
        ...
        neighbors_dict = [node001: [node366, node362, node002],
                          ...
                          node1007: [node5006, node233, node230, node204 /*node1008*/],
                          ...
                          node204: [node203, node206, node1007],
        ...
        for (node,neighbors) in neighbors_dict {
            addNeighbors(sourceNode: node, array: neighbors) //establish the connections between rooms
            print(neighbors)
        }
    ...
    @IBAction func goButtonClicked(_ sender: UIButton) {
        myGraph.nodes!.flatMap({ $0 as? MyNode}).forEach { node in //run through myGraph
            nodeNameDict[node.name] = node //add the node to the dict [String:MyNode] --used to lookup node given string entered in text field
        }
        ...
        if nodeNameDict[destTextField.text!] != nil && nodeNameDict[sourceTextField.text!] != nil{ //check if the entries are valid
            ...
            if sourceNode.getName() == "360" && latitudes_first.keys.contains(destNode.getName()){ //take care of the case where room 360 goes upstairs, then downstairs again during 360-old pit room
              sourceNode.removeConnection(to: node066) //this is bidirectional by default
            }
                ...
                destNode.removeConnection(to: node066) //remove 066 (stair access node from 360's connections)
            }
            ...
            makeRoutePoints(array:routePath) //changes the arrays firstFloorPoints, secondFloorPoints
            print("firstfloorpoints")
            print(firstFloorPoints)

            errorMessage.text = nil
            
            node360.addConnection(to: node066, weight: 43) //reconnect 066 to 360 if it was previously removed so that the next time someone wants to go from 360-470, the stairs are available (this is after making the route and coordinates, so it still avoids taking you upstairs on a first floor-first floor route
